...
pair<int,int> Player::getMinMaxAB(int depth, int evalFunc, int player, pair<int,int> position){
	...
	if(player == 1){
		...
		tn1 += n1;
		tne1 += ne1;
		/*cout << "Total nodes generated for Player 1: " << tn1 << endl;
		cout << "Total nodes expanded for Player 1: " << tne1 << endl;*/
		...
		tne2 += ne2;
		pair<int,vector<pair<int,int>>> mv = minMaxAB(position,0,player,INT_MAX,INT_MIN,evalFunc);
		/*cout << "Total nodes generated for Player 2: " << tn2 << endl;
		cout << "Total nodes expanded for Player 2: " << tne2 << endl;*/
		return mv.second.back();
	}
}
...
int Player::e3(int pname, pair<int,int> possibleMove){
	...
	if(pname == 1){
	   ...
	   if(possibleMove.second >= 4){
	    ...
	    for(int j = 1;j<4;j++){
	    if(possibleMove.first < 3){
	            if((b1[possibleMove.first+j][possibleMove.second])== "X"){
	                score += 25;
	            }
	        }
	    }
	...
	// diagonals
	  for(int j = 1;j<4;j++){
	    if(possibleMove.first > 2 && possibleMove.second < 4){
	            if((b1[possibleMove.first-j][possibleMove.second+j])== "X"){
	                score+=25;
	             }
	    }
	    else if(possibleMove.first < 3 && possibleMove.second <4){
	     ...
	     for(int j = 1;j<4;j++){
	    if(possibleMove.second < 4){
	            if((b1[possibleMove.first][possibleMove.second+j])== "O"){
	                score += 25;
	            }
	        }
	    }
	    ...
	    for(int j = 1;j<4;j++){
	    if(possibleMove.first < 3){
	            if((b1[possibleMove.first+j][possibleMove.second])== "O"){
	                score += 25;
	            }
	        }
	    }
	...
	for(int j = 1;j<4;j++){
	    if(possibleMove.first > 2 && possibleMove.second < 4){
	            if((b1[possibleMove.first-j][possibleMove.second+j])== "O"){
	                score+=25;
	             }
	    }
	    else if(possibleMove.first < 3 && possibleMove.second <4){
...
int Player::getTotalNodesGenerated(int playerName){
	if(playerName == 1)
		return tn1;
	else 
		return tn2;
}
int Player::getTotalNodesExpanded(int playerName){
	if(playerName == 1)
		return tne1;
	else 
		return tne2;
...
double Player::memoryUsed(int nodeCount) {
    return 8 * nodeCount / 1024.0;
}